...
*** Applications
    ...
    Currently, each infix operator is a sibling of the element to the left and the right.  Instead,
    ...
    Okay, maybe not the best rendering, but it'll do in a pinch.  We can see that the operators are
    now a level up from the things they're operating on, but we're no longer in DFS preorder.  Does
    this matter?  Not necessarily.  We can still iterate through the tree.  In fact, we can iterate
    through the tree to generate the DFS preorder.
    ...
    the parent vector just as we do the values vector:
    ...
    a paren after each operator and use the parser above.  Another is not note that each operator has
    ...
    basically a term rewriting exercise.  Which means a lot of frequent updates to the tree and
    frequent changes in the size of the trees, which means a lot of copying of the vectors
    involved as described above.  It works, but perhaps is not the best fit.
