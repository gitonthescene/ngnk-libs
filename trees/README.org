...
** Apter trees
   ...
   We could also look at the fixed point scan: ~(p@)\!#p~ which gives a matrix whose columns give paths
...
*** Depth vectors
	...
	The depth vector is simply the length of each path from a leaf up to the parent.  I.e. ~+/1&(p@)\!#p:0^p~.
